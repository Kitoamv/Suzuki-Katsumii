...
exports.queueSong = (message, searchString) => {
    ...
    ytclient.search(params, function(err, data) {
        ...
        if(data.items.length == 0){
            message.channel.send("<:warning:408740166715310100> Nenhuma música encontrada");
            return;
        }
        ...
        YTDL.getInfo(vidId, (err, info) => {
            ...
            var embed = new Discord.RichEmbed()
                .setColor(9955331)
                .setDescription(`<:musical_note:408759580080865280> **Música em fila:** [${song.title}](${ytVidUrl}) | ${vidLcodeTags}`);

            message.channel.send(embed);

            g.playQueue.push(song);
            console.log("--> Música em fila: " + song.title + " (" + vidId + ")");

            // join voice channel if not in one already
            if(!message.guild.voiceConnection){
                message.member.voiceChannel.join().then((connection) => {
                    playSong(connection, guildId);
                    console.log("--> Canal de voz associado: "
...
async function playSong(connection, guildId){
    ...
    console.log("--> Começou a tocar música: " + g.playQueue[0].title
                                        + " (" + g.playQueue[0].id + ")");
    ...
    g.dispatcher.on("end", end => {
        ...
        if(g.playQueue[0]){
            ...
            console.log("--> Saindo do canal de voz: " + connection.channel.name);
        }
    });
}
...
exports.playQueue = (guildId, channel) => {
    ...
    if(!guilds[guildId] || !guilds[guildId].nowPlaying){
        var embed = new Discord.RichEmbed()
            .setColor(9955331)
            .setDescription("<:mute:456503709443293186> Não jogar");
        channel.send(embed);
        return;
    }
    ...
    var embed = new Discord.RichEmbed()
        .setColor(9955331)
        .addField("<:musical_note:408759580080865280> Tocando agora", cs);
        if(g.loop)  embed.setFooter("🔁 Lista de reprodução em loop");
        if(q != "") embed.addField("<:notes:433601162827137026> Fila do Google Play", q);
...
exports.toggleLoop = (guildId, channel) => {
    ...
    if(g.loop){
        channel.send("<:musical_note:408759580080865280> Lista de reprodução em loop");
    }else{
        channel.send("<:musical_note:408759580080865280> Desativado o loop")
    }
};
