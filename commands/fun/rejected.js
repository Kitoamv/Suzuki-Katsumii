...
module.exports = {
        name: "rejected",
        aliases: [],
        description: "Você vai rejeitar uma pessoa.",
        ussage: "<prefix>rejected <mention>",
        category: "fun",
  ...
  if (!message.guild.member(client.user).hasPermission('ATTACH_FILES')) return message.reply('Desculpe, eu não tenho os privilégios para fazer este comando que eu preciso ATTACH_FILES. :x:')
   const { Canvas } = require('canvas-constructor');
    if (message.mentions.users.size < 1) return message.channel.send("Você precisa marcar um usuário para rejeitar.");
